...
for i in range(1,k+1):
	...
	NNclf = MLPClassifier(hidden_layer_sizes=(50,30))
	NNclf.fit(DataTrain,DataTrainRes)

	for i in range(len(DataTest)):
		NNres = NNclf.predict(DataTest[i])
		if NNres[0] == DataTestRes[i]:
			successCount += 1
...
for i in range(len(DataTest)):
	...
	if NNres[0] == DataTestRes[i]:
		successCount += 1
	confusionMatrix[DataTestRes[i]+1][NNres[0]+1] += 1
	confusionMatrix[-1][NNres[0]+1] += 1
